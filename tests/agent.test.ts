...
describe("postReviewComment", () => {
  it("appends model and metrics footer", async () => {
    ...
    expect(capturedBody).toContain("- Model: `openai/gpt-5`");
...
describe("postReviewStructured", () => {
  it("does not delete old Hodor comments in hybrid mode", async () => {
    capturedExecCalls = [];
    ...
    expect(flattenedArgs.some((arg) => arg.includes("/notes?per_page="))).toBe(true);
...
describe("getHodorReviewShaCandidates", () => {
  ...
  it("uses updated_at when a rolling summary keeps its original creation time", () => {
    const shas = getHodorReviewShaCandidates([
      {
        body: `<!-- hodor:sha:${SHA_NEW} -->\nrolling`,
